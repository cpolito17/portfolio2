...
/* ═══ 0. Render options ═════════════════════════════════════
   The astronomy is fixed; the rendering is not. A variant sets window.SKY_OPTS
   before this script runs to change the framing, the dither, the palette, the
   Earth, and how the planets are drawn. Every default below is the value this
   engine used when it had only one caller, so an empty SKY_OPTS is a no-op. */
const OPT = Object.assign({
  viewAlt: 38, viewAz: 178, halfFov: 70,   // degrees: where the camera looks
  bayer: 8,                                 // ordered-dither matrix size, 2/4/8/16
  ditherDepth: 0,                           // >0 coarsens the matrix toward the edge
  skyRamp:   [[5,6,13],[10,13,24],[16,21,41],[24,32,60],[33,44,82],[43,56,104]],
  glowRamp:  [[207,226,255],[143,184,238],[90,134,200],[56,87,143],[37,58,99]],
  earthRamp: [[12,22,44],[8,15,32],[5,10,22],[3,6,14]],
  limb: { top: 0.84, r: 2.35, glow: 0.034 }, // fractions of canvas width; null = no Earth
  planets: 'dot',                            // 'dot' | 'disc' | 'off'
  planetScale: 1,
  labels: '#grid',                           // element the callouts route around; null = off
  sats: 2, fps: 12,
  starGain: 1,
  maxW: 560, resDiv: 3.2                     // internal buffer size
}, window.SKY_OPTS || {});
...
const VIEW_AZ = OPT.viewAz*DEG, VIEW_ALT = OPT.viewAlt*DEG, HALF_FOV = OPT.halfFov*DEG;
...
/* ═══ 4. Dithered sky ═══════════════════════════════════════ */
/* The ordered-dither matrix, generated by the standard recurrence
   M(2n) = [[4M, 4M+2], [4M+3, 4M+1]] rather than written out, so the size is a
   variant option. bayer(8) reproduces the 8x8 table this file used to hold,
   cell for cell. */
function bayer(n){
  let m = [[0]];
  while (m.length < n){
...
const BAYER = bayer(OPT.bayer), BMASK = OPT.bayer - 1;
const SKY_RAMP = OPT.skyRamp;
/* Atmospheric limb: bright rim fading outward into the star field. */
const GLOW_RAMP = OPT.glowRamp;
const EARTH_RAMP = OPT.earthRamp;
...
const LIMB = OPT.limb;
const LIMB_TOP = LIMB ? LIMB.top : 0, LIMB_R = LIMB ? LIMB.r : 0,
      GLOW_W = LIMB ? LIMB.glow : 0;
...
function buildBackdrop(){
  ...
  CITY = [];
  const DD = OPT.ditherDepth;
  const R = LIMB_R*W, gw = GLOW_W*W;
  const cxE = CX, cyE = LIMB_TOP*H + R;          // limb centre sits far below frame
  for (let py=0; py<H; py++){
    for (let pxi=0; pxi<W; pxi++){
      /* With ditherDepth the matrix cell grows with distance from centre, so
         the grain coarsens outward and the field reads as having depth. */
      const q = DD ? 1 + ((Math.hypot(pxi - CX, py - CY)/CX * DD) | 0) : 1;
      const t = BAYER[((py/q)|0) & BMASK][((pxi/q)|0) & BMASK];
      const dE = LIMB ? Math.hypot(pxi - cxE, py - cyE) : Infinity;
      let c;
      if (dE < R){                                  // ── the planet
        ...
        /* Without an Earth there is no glow to blend out of, so the ramp is
           driven by altitude alone. */
        const fade = LIMB ? Math.min(1, (dE - R - gw)/(gw*2.6)) : 1;
...
function resize(){
  ...
  W = Math.min(OPT.maxW, Math.round(vw/OPT.resDiv));
...
function drawStars(now){
  for (let i=0;i<STARS.length;i++){
    ...
    let b = Math.pow(Math.max(0, (5.7 - s.mag)/6.6), 1.1) * OPT.starGain;
...
/* Two planet treatments over the same positions. 'dot' is a 2-5px square with a
   thin cross, which is what a bright point looks like at this resolution. 'disc'
   resolves the body instead: an ordered-dithered circle with a bloom falling off
   around it, which is not what the eye sees from the ground but is what the
   medium can actually draw at a size worth looking at. */
const BODY_SIZE = { Sun: 5, Jupiter: 3, Venus: 3 };
const bodySize = n => (BODY_SIZE[n] || 2);

function drawDot(b, sx, sy){
  const big = bodySize(b.name);
...
function drawDisc(b, sx, sy){
  ...
  for (let py=y0; py<=y1; py++){
    for (let px=x0; px<=x1; px++){
      ...
      if (d > bloom) continue;
      if (occluded(px, py)) continue;
      const t = BAYER[py & BMASK][px & BMASK];
      if (d <= r){                                   // the body itself
        const shade = d/r;                           // limb-darkened toward the edge
        const lvl = Math.min(DISC_RAMP.length-1, (shade*(DISC_RAMP.length-1) + t*0.9)|0);
        const c = DISC_RAMP[lvl];
        ctx.fillStyle = `rgb(${c[0]},${c[1]},${c[2]})`;
      } else {                                       // bloom, dithered to nothing
        const f = 1 - (d-r)/(bloom-r);
        if (f*f < t) continue;                       // the dither IS the falloff
        ctx.fillStyle = `rgba(255,222,170,${(0.5*f).toFixed(3)})`;
      }
...
function drawBodies(){
  if (OPT.planets === 'off'){ layoutLabels([]); return; }
  const placed = [];
  const paint = OPT.planets === 'disc' ? drawDisc : drawDot;
...
function layoutLabels(items){
  if (!OPT.labels){ svg.textContent = ''; lastLabelKey = ''; return; }
  const gridEl = OPT.labels ? document.querySelector(OPT.labels) : null;
...
function drawSats(){
  ...
  if (OPT.sats && sats.length < OPT.sats + 3) spawnSat();   // keep something crossing
}
...
resize(); recomputeSky();
for (let i=0;i<OPT.sats;i++) spawnSat();
requestAnimationFrame(loop);
